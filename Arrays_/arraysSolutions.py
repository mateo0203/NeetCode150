...
class Solutions:
    ...
    def rotate(self, nums: list[int], k: int) -> None:
        ...
        nums = [1,2,3,4,5,6,7], k = 3, k is non-negative
            [5, 6, 7, 1, 2, 3, 4]

            use modulo indexing, to change the indices of each for example
            [1, 3, 5, 2], k = 2 / 1, easy currIdx = 0 + 2, nIdx = 2 / 2 currIdx = (4) % 4 = 1

            nIdx = (currIdx + k) % len(nums)
            From
            [1, 3, 5, 2]
                        To: [7, 2, 1, 3, 5]
            [7, 2, 1, 3, 5]

            Use a visited set to not repeat indices

            It is O(n) time complexity and O(1) space 
            """
        rotations = 0
        for i in range(len(nums)):
            #check if rotations num complete
            if rotations >= len(nums):
                break
            idxToRotate = i
            valueToRotate = nums[i]
            while rotations != len(nums):
                nIndex = (idxToRotate + k) % len(nums)
                idxToRotate = nIndex
                tmp = nums[nIndex]
                nums[nIndex] = valueToRotate
                valueToRotate = tmp
                rotations += 1
